    ...
    public class ReportesController : ControllerBase
    {
            ...
            [FromQuery] string? anio)
        {
            ...
            string query = $@"SELECT 
                ...
                v.persona_visitar
                FROM visitas v
                LEFT JOIN entradas_salidas es ON es.visita_id = v.id
                {whereClause}
            ...
            while (await reader.ReadAsync())
            {
                registros.Add(new {
                    ...
                    fechaEntrada = reader["fecha"] == DBNull.Value ? null : Convert.ToDateTime(reader["fecha"]).ToString("dd/MM/yyyy") + " " + reader["hora_entrada"].ToString(),
